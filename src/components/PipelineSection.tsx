...
const DOMAINS = [
  { id: 'clinic' as DomainId, label: 'General Clinic', color: 'var(--salvia-accent)' },
  { id: 'vet' as DomainId, label: 'Veterinary', color: 'var(--accent-vet)' },
  { id: 'dental' as DomainId, label: 'Dental', color: 'var(--accent-outcomes)' },
];

const STATUS = {
  done: { color: 'var(--accent-dental)', bg: '#ECFDF5', label: 'Processed' },
  flag: { color: '#D97706', bg: '#FFFBEB', label: 'Flagged' },
  locked: { color: 'var(--salvia-primary)', bg: '#F8FAFC', label: 'Locked' },
};
...
const FormFields = ({ fields }: { fields: [string, string][] }) => (
  <div style={{ display: 'flex', flexDirection: 'column', gap: '0.3rem' }}>
    {fields.map(([label, value], i) => (
      <div key={i} style={{ display: 'flex', gap: '0.5rem', alignItems: 'baseline', paddingBottom: '0.3rem', borderBottom: i < fields.length - 1 ? '1px solid var(--border-subtle)' : 'none' }}>
        <span style={{ fontSize: 'var(--text-2xs)', fontWeight: 600, color: '#94A3B8', textTransform: 'uppercase', letterSpacing: '0.05em', minWidth: 76, flexShrink: 0 }}>{label}</span>
        <span style={{ fontSize: 'var(--text-xs)', color: 'var(--salvia-primary)', fontWeight: 500 }}>{value}</span>
      </div>
    ))}
  </div>
);
...
const PolicyItems = ({ items }: { items: [string, 'ok' | 'warn'][] }) => (
  <div style={{ display: 'flex', flexDirection: 'column', gap: '0.28rem' }}>
    {items.map(([label, state], i) => (
      <div key={i} style={{ display: 'flex', alignItems: 'center', gap: '0.5rem', paddingBottom: '0.28rem', borderBottom: i < items.length - 1 ? '1px solid var(--border-subtle)' : 'none' }}>
        <div style={{
          width: 18, height: 18, borderRadius: 'var(--radius-sm)', flexShrink: 0,
          backgroundColor: state === 'ok' ? '#ECFDF5' : '#FFFBEB',
          color: state === 'ok' ? 'var(--accent-dental)' : 'var(--salvia-warning)',
          fontSize: 'var(--text-2xs)', fontWeight: 700,
          display: 'flex', alignItems: 'center', justifyContent: 'center',
        }}>
          {state === 'ok' ? '✓' : '!'}
        </div>
        <span style={{ fontSize: 'var(--text-xs)', color: state === 'warn' ? '#92400E' : '#334155', fontWeight: state === 'warn' ? 500 : 400 }}>{label}</span>
      </div>
    ))}
  </div>
);
...
const SignOff = ({ name, role, note }: { name: string; role: string; note: string }) => (
  <div>
    <div style={{ display: 'flex', alignItems: 'center', gap: '0.6rem', marginBottom: '0.5rem' }}>
      <div style={{
        width: 30, height: 30, backgroundColor: 'var(--salvia-primary)', color: '#fff',
        fontSize: 'var(--text-2xs)', fontWeight: 700, borderRadius: '50%', flexShrink: 0,
        display: 'flex', alignItems: 'center', justifyContent: 'center',
      }}>
        {name.replace('Dr. ', '').split(' ').map(w => w[0]).join('').slice(0, 2)}
      </div>
      <div style={{ flex: 1 }}>
        <div style={{ fontSize: 'var(--text-xs)', fontWeight: 600, color: 'var(--salvia-primary)', letterSpacing: '-0.01em' }}>{name}</div>
        <div style={{ fontSize: 'var(--text-2xs)', color: '#94A3B8' }}>{role}</div>
      </div>
      <div style={{
        fontSize: 'var(--text-2xs)', fontWeight: 700, color: 'var(--accent-dental)',
        backgroundColor: '#ECFDF5', padding: '0.2rem 0.55rem',
        borderRadius: 'var(--salvia-radius-full)', letterSpacing: '0.02em',
      }}>Signed</div>
    </div>
    <div style={{ fontSize: 'var(--text-xs)', color: 'var(--salvia-text-muted)', lineHeight: 1.6, paddingTop: '0.45rem', borderTop: '1px solid var(--border-subtle)' }}>{note}</div>
  </div>
);
...
const SCENARIOS: Record<DomainId, DomainScenario> = {
  clinic: {
    top: {
      color: 'var(--salvia-accent)', tag: 'AUDIO', title: 'Voice note captured',
    ...
    bottom: {
      color: 'var(--accent-dental)', tag: 'PUBLISH', title: 'Staff review & publish',
  ...
  vet: {
    top: {
      color: 'var(--accent-vet)', tag: 'AUDIO', title: 'Voice note captured',
  ...
  dental: {
    top: {
      color: 'var(--accent-outcomes)', tag: 'AUDIO', title: 'Voice note captured',
    ...
    bottom: {
      color: 'var(--accent-outcomes)', tag: 'PUBLISH', title: 'Staff review & publish',
  ...
  <div style={{
    ...
    <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '0.8rem' }}>
      <div style={{ display: 'flex', alignItems: 'center', gap: '0.5rem' }}>
        <div style={{ width: 10, height: 10, backgroundColor: n.color, borderRadius: 'var(--radius-sm)', flexShrink: 0 }} />
        <span style={{ fontSize: 'var(--text-sm)', fontWeight: 600, color: 'var(--salvia-primary)', letterSpacing: '-0.01em' }}>{n.title}</span>
      </div>
      <span style={{
        fontSize: 'var(--text-2xs)', fontWeight: 700, letterSpacing: '0.06em',
        color: n.color, backgroundColor: `${n.color}12`,
        padding: '0.18rem 0.5rem', borderRadius: 'var(--salvia-radius-full)',
      }}>
        {n.tag}
      </span>
    </div>
...
const Connector = ({ badge, color }: { badge: string; color: string }) => (
  <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'center' }}>
    ...
    <div style={{
      ...
      padding: '0.18rem 0.65rem', borderRadius: 'var(--salvia-radius-full)',
...
export const PipelineSection = () => {
  ...
  return (
    <section id="pipeline" style={{ backgroundColor: 'var(--salvia-bg)', padding: 'var(--section-pad) 0' }}>
      <style>{`
      ...
      <div className="container" style={{ maxWidth: '1200px' }}>
        ...
        <div style={{ textAlign: 'center', marginBottom: '3rem', maxWidth: '720px', marginInline: 'auto' }}>
          ...
          <p style={{ color: 'var(--salvia-text-muted)', fontSize: 'var(--text-md)', lineHeight: 1.6, marginTop: '1rem' }}>
          ...
          {DOMAINS.map(d => (
            <button
              ...
              style={{
                ...
                whiteSpace: 'nowrap',
                ...(active === d.id
                  ? { backgroundColor: 'var(--salvia-primary)', color: '#fff', boxShadow: 'var(--shadow-2)' }
                  : { backgroundColor: '#fff', color: 'var(--salvia-text-muted)', boxShadow: 'var(--shadow-2), inset 0 0 0 1px rgba(0,0,0,0.08)' }
                ),
              }}
            >
              {d.label}
        ...
        <div style={{ display: 'grid', gridTemplateColumns: '1.1fr 1fr', gap: '2rem', alignItems: 'start' }} className="mobile-stack">
          ...
          <div style={{
            ...
            <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', marginBottom: '0.6rem' }}>
              <div style={{ display: 'flex', alignItems: 'center', gap: '0.4rem', marginBottom: '0.35rem' }}>
                <div style={{ width: 8, height: 8, borderRadius: '50%', backgroundColor: 'var(--accent-dental)', boxShadow: '0 0 0 3px #DCFCE7' }} />
                <span style={{ fontSize: 'var(--text-2xs)', fontWeight: 600, color: 'var(--accent-dental)', letterSpacing: '0.04em', textTransform: 'uppercase' }}>Start</span>
              </div>
          ...
          <div style={{
            ...
            <div style={{ padding: '1rem 1.2rem', borderBottom: '1px solid var(--border-subtle)', display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
              <div>
                <div style={{ fontSize: 'var(--text-sm)', fontWeight: 600, color: 'var(--salvia-primary)', letterSpacing: '-0.01em' }}>Audit trail</div>
                <div style={{ fontSize: 'var(--text-2xs)', color: '#94A3B8', marginTop: '1px' }}>Immutable · Hashed on lock</div>
              </div>
              <div style={{ fontSize: 'var(--text-2xs)', fontWeight: 600, color: domain.color, backgroundColor: `${domain.color}12`, padding: '0.22rem 0.65rem', borderRadius: 'var(--salvia-radius-full)' }}>
                ...
                return (
                  <div key={`${active}-${i}`} style={{ display: 'flex', padding: '0 1.2rem' }}>
                    <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', width: 16, flexShrink: 0, marginRight: '0.75rem' }}>
                      ...
                      {!isLast && <div style={{ width: 1.5, flex: 1, backgroundColor: 'var(--border-subtle)', margin: '3px 0 0' }} />}
                    </div>
                      ...
                      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', gap: '0.4rem' }}>
                        <div>
                          <div style={{ display: 'flex', alignItems: 'center', gap: '0.35rem', marginBottom: '0.18rem', marginTop: '0.9rem' }}>
                            <span style={{ fontSize: 'var(--text-2xs)', fontWeight: 600, color: '#94A3B8', fontFamily: 'monospace' }}>{e.time}</span>
                            <span style={{ color: 'var(--border-strong)', fontSize: 'var(--text-2xs)' }}>·</span>
                            <span style={{ fontSize: 'var(--text-2xs)', fontWeight: 500, color: 'var(--salvia-text-muted)' }}>{e.actor}</span>
                          </div>
                          <p style={{ margin: 0, fontSize: 'var(--text-xs)', color: '#334155', lineHeight: 1.5 }}>{e.action}</p>
                        </div>
                        <span style={{
                          fontSize: 'var(--text-2xs)', fontWeight: 600,
                          color: st.color, backgroundColor: st.bg,
                          padding: '0.2rem 0.5rem', borderRadius: 'var(--salvia-radius-full)',
            ...
            <div style={{ padding: '0.8rem 1.2rem', borderTop: '1px solid var(--border-subtle)', display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
              <span style={{ fontSize: 'var(--text-2xs)', color: '#94A3B8' }}>Tamper-proof · hashed on lock</span>
              <button className="pill-button" style={{ fontSize: 'var(--text-2xs)', padding: '0.4rem 0.9rem', display: 'flex', alignItems: 'center', gap: '0.3rem' }}>
