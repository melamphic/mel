...
const Card: React.FC<{ accent: string; span: string; eyebrow: string; title: string; desc: string; children: React.ReactNode }> = ({ accent, span, eyebrow, title, desc, children }) => (
  <div style={{ gridColumn: span, background: '#fff', border: `1px solid ${LINE}`, borderRadius: 20, padding: '1.9rem', display: 'flex', flexDirection: 'column', boxShadow: cardShadow, position: 'relative', overflow: 'hidden' }}>
    ...
    <div style={{ display: 'flex', alignItems: 'center', gap: '0.5rem', marginBottom: '0.55rem' }}>
      ...
      <span style={{ fontSize: 'var(--text-2xs)', fontWeight: 800, letterSpacing: '0.12em', textTransform: 'uppercase', color: accent }}>{eyebrow}</span>
    </div>
    <div style={{ fontSize: 'var(--text-md)', fontWeight: 800, color: INK, letterSpacing: '-0.02em' }}>{title}</div>
    <p style={{ fontSize: 'var(--text-sm)', color: MUTED, lineHeight: 1.55, margin: '0.45rem 0 1.4rem', maxWidth: 360 }}>{desc}</p>
...
const FieldRow: React.FC<{ label: React.ReactNode; value: React.ReactNode; last?: boolean }> = ({ label, value, last }) => (
  <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', padding: '0.6rem 0', borderBottom: last ? 'none' : `1px solid ${LINE}` }}>
    <span style={{ display: 'flex', alignItems: 'center', gap: '0.5rem', fontSize: 'var(--text-xs)', color: INK, fontWeight: 600 }}>{label}</span>
    <span style={{ fontSize: 'var(--text-xs)', color: MUTED }}>{value}</span>
  </div>
);

const Tag: React.FC<{ t: string; c: string }> = ({ t, c }) => (
  <span style={{ fontSize: 'var(--text-2xs)', fontWeight: 800, letterSpacing: '0.04em', color: c, background: `${c}1A`, padding: '0.18rem 0.5rem', borderRadius: 6 }}>{t}</span>
);
...
const Status: React.FC<{ ok?: boolean; warn?: boolean; label: string }> = ({ ok, warn, label }) => {
  ...
  return (
    <span style={{ display: 'inline-flex', alignItems: 'center', gap: '0.35rem', fontSize: 'var(--text-2xs)', fontWeight: 700, color: c, background: bg, padding: '0.3rem 0.6rem', borderRadius: 999 }}>
...
export const BentoReinvented: React.FC = () => {
  ...
  <section id="outputs" style={{ padding: 'clamp(5.5rem, 9vw, 8rem) 0', backgroundColor: 'transparent', position: 'relative', zIndex: 10 }}>
    ...
    <div className="container" style={{ maxWidth: '760px', textAlign: 'center', marginBottom: '3.5rem' }}>
      <span style={{ fontSize: 'var(--text-2xs)', fontWeight: 800, letterSpacing: '0.18em', textTransform: 'uppercase', color: 'var(--salvia-accent)' }}>The outputs</span>
        ...
        <Card span="span 7" accent="#FF4E00" eyebrow="Inside a form" title="Typed widgets, not paragraphs" desc="Consent, drugs, pain and witness captured as structured records — assembled live as the clinician speaks.">
          <div style={{ background: '#FBFAF8', border: `1px solid ${LINE}`, borderRadius: 14, padding: '1.1rem 1.25rem' }}>
            <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: '0.5rem' }}>
              <span style={{ fontSize: 'var(--text-sm)', fontWeight: 800, color: INK }}>Surgical consent</span>
              <Tag t="WITNESSED" c="#0EA5E9" />
            </div>
            <p style={{ fontSize: 'var(--text-xs)', color: MUTED, lineHeight: 1.5, margin: '0 0 0.4rem' }}>
              {isIndia
            ...
            <FieldRow label={<>Tramadol 50&thinsp;mg <Tag t={isIndia ? 'SCH H1' : 'CD'} c="#DC2626" /></>} value={<span style={{ fontFamily: 'monospace', color: 'var(--salvia-success)', fontWeight: 700 }}>bal 18 → 16</span>} />
            ...
            {[
              ...
              <div key={r.clause} style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', gap: '0.6rem', background: '#FBFAF8', border: `1px solid ${LINE}`, borderRadius: 12, padding: '0.7rem 0.85rem' }}>
                <span style={{ display: 'flex', alignItems: 'center', gap: '0.55rem', minWidth: 0 }}>
                  <Tag t={r.tag} c={r.tc} />
                  <span style={{ fontSize: 'var(--text-xs)', fontWeight: 600, color: INK, whiteSpace: 'nowrap', overflow: 'hidden', textOverflow: 'ellipsis' }}>{r.clause}</span>
                </span>
        ...
        <Card span="span 5" accent="#7C3AED" eyebrow="Outcomes" title="Trends across the episode" desc="NPRS and recovery measures tracked visit-over-visit.">
          <div style={{ background: '#FBFAF8', border: `1px solid ${LINE}`, borderRadius: 14, padding: '1.1rem 1.25rem' }}>
            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'baseline', marginBottom: '0.7rem' }}>
              <span style={{ fontSize: 'var(--text-xs)', fontWeight: 800, color: INK }}>Pain · NPRS · 4 weeks</span>
              <span style={{ fontSize: 'var(--text-sm)', fontWeight: 800, color: 'var(--salvia-success)' }}>−67%</span>
            </div>
            ...
            <div style={{ display: 'flex', justifyContent: 'space-between', marginTop: '0.5rem', fontSize: 'var(--text-2xs)', color: FAINT, fontFamily: 'monospace' }}>
        ...
        <Card span="span 7" accent="#D97706" eyebrow="Registers" title="Audit-ready, reconciled, exportable" desc="Controlled-drug register, vaccine log and incident log build themselves — one-click export for the assessor.">
          <div style={{ background: '#FBFAF8', border: `1px solid ${LINE}`, borderRadius: 14, padding: '1.1rem 1.25rem' }}>
            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '0.7rem' }}>
              <span style={{ fontSize: 'var(--text-xs)', fontWeight: 800, color: INK }}>Controlled drug register · June</span>
              <span style={{ display: 'inline-flex', alignItems: 'center', gap: '0.3rem', fontSize: 'var(--text-2xs)', fontWeight: 800, color: 'var(--salvia-success)', background: '#F0FDF4', padding: '0.25rem 0.6rem', borderRadius: 999 }}>
            ...
            <div style={{ display: 'grid', gridTemplateColumns: '1.5fr 1fr 0.8fr 0.8fr 0.5fr', columnGap: '0.6rem', rowGap: '0.55rem', fontSize: 'var(--text-xs)', alignItems: 'center' }}>
              {['Drug', 'Batch', 'Given', 'Balance', 'Sch'].map((h) => <span key={h} style={{ color: FAINT, fontWeight: 800, fontSize: 'var(--text-2xs)', letterSpacing: '0.06em', textTransform: 'uppercase' }}>{h}</span>)}
              {[
